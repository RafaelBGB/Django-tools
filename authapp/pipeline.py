...
def save_user_profile(backend, user, response, *args, **kwargs):
    if backend.name == 'vk-oauth2':
        ...
        if 'photo' in response.keys():
            with open(f'{MEDIA_ROOT}/user_avatars/{response["first_name"]}_'
                      f'{response["last_name"]}.jpg', 'wb') as f:
                f.write(requests.get(response['photo']).content)

            user.avatar = f'{MEDIA_ROOT}/user_avatars/{response["first_name"]}_' \
                          f'{response["last_name"]}.jpg'
    user.save()
